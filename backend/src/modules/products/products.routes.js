...
const routes = [
  {
    ...
    path: '/',
    auth: 'any',
    summary: 'Browse catalog products with filtering, search, and keyset pagination',
  ...
  },
  {
    ...
    path: '/:id/reviews',
    auth: 'any',
    summary: 'List customer reviews for a specific product',
    handler: async (req, res, next) => {
      try {
        const limit = parseLimit(req.query.limit, 20, 50);
        const cursor = req.query.cursor ? String(req.query.cursor).trim() : undefined;
  ...
  },
  {
    ...
    path: '/:id/related',
    auth: 'any',
    summary: 'Get related products from farmer and matching categories',
    handler: async (req, res, next) => {
      try {
...
defineRoutes(productsRouter, 'products', routes, { basePath: '/api/products' });
