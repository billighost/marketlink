...
const routes = [
  ...
  // PATCH /api/users/me - Update permitted profile fields
  {
    ...
    path: '/me',
    auth: 'any',
    summary: 'Update permitted profile fields',
  ...
  // DELETE /api/users/me/sessions - Sign out of all devices
  {
    method: 'delete',
    path: '/me/sessions',
    auth: 'any',
    summary: 'Sign out of all devices and revoke all refresh sessions',
    handler: async (req, res) => {
      await revokeAllUserSessions(req.user.id);
  ...
  {
    method: 'get',
    path: '/me/saved-markets',
    auth: 'any',
    summary: 'List saved markets for authenticated user',
    handler: async (req, res) => {
  ...
  {
    method: 'put',
    path: '/me/saved-markets/:marketId',
    auth: 'any',
    summary: 'Save a market to bookmarks (max 10)',
    handler: async (req, res) => {
  ...
  // DELETE /api/users/me/saved-markets/:marketId
  {
    ...
    path: '/me/saved-markets/:marketId',
    auth: 'any',
    summary: 'Remove market from saved bookmarks',
    handler: async (req, res) => {
      if (!isValidObjectId(req.params.marketId)) {
        throw AppError.notFound('Market not found.');
      }
  ...
  {
    method: 'put',
    path: '/me/home-market/:marketId',
    auth: 'any',
    summary: 'Set primary home market',
    handler: async (req, res) => {
      if (!isValidObjectId(req.params.marketId)) {
        throw AppError.notFound('Market not found.');
      }
...
defineRoutes(usersRouter, 'users', routes, { basePath: '/api/users' });
