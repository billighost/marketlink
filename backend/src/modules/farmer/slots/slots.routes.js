...
const routes = [
  // GET /api/farmer/slots?days=14
  {
    ...
    path: '/',
    auth: 'farmer',
    summary: 'List upcoming pickup slots with capacity and order counts',
  ...
  // PUT /api/farmer/slots/closures
  {
    ...
    summary: 'Mark dates closed for pickup',
    body: 'slotClosures',
    handler: async (req, res, next) => {
      try {
  ...
  // DELETE /api/farmer/slots/closures/:date
  {
    ...
    auth: 'farmer',
    middlewares: [requireNotSuspendedFarmer],
    summary: 'Reopen a previously closed pickup date',
    handler: async (req, res, next) => {
      try {
...
defineRoutes(farmerSlotsRouter, 'farmerSlots', routes, { basePath: '/api/farmer/slots' });
